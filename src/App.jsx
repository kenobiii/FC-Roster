...
function App() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      if (saved.subs)        setSubs(saved.subs);
    }
  }, []);
  ...
  return (
    <div className="flex flex-col" style={{ minHeight:"100dvh", width:"100%", background:BRAND.colors.navy,
      ...
      <header style={{ background:"#fdfef8", borderBottom:"1px solid rgba(0,0,0,0.08)" }}>
          ...
          <div className="ml-auto shrink-0">
            {session ? (
              <button onClick={e => { e.stopPropagation(); setShowProfile(v => !v); }}
                ...
                <span style={{ fontSize:16 }}>{profile?.avatar_emoji || "⚽"}</span>
                <span className="hidden sm:inline">{profile?.display_name || session.email?.split("@")[0]}</span>
              </button>
            ) : (
              <button onClick={e => { e.stopPropagation(); setShowAuth(true); }}
                className="px-4 py-1.5 rounded-xl text-xs font-black tracking-wide transition-all hover:brightness-110"
                style={{ background:BRAND.colors.yellow, color:"#111" }}>
                SIGN IN
              </button>
            )}
          </div>
        ...
        {/* Tab row */}
        <div className="flex gap-1.5 px-4 pb-3 pt-2 overflow-x-auto"
          style={{ borderTop:"1px solid rgba(0,0,0,0.08)", background:"#e8eaed" }}>
          {TABS.map(({ id, label }) => (
        ...
        {activeTab === "builder" && (
          <>
            <div className="flex items-end gap-3 px-4 pt-2.5 pb-2.5 flex-wrap"
              style={{ borderTop:"1px solid rgba(0,0,0,0.12)", background:"#d4d6da" }}>
            ...
            <div className="flex items-center gap-3 px-4 pt-2 pb-2"
              style={{ borderTop:"1px solid rgba(0,0,0,0.1)", background:"#caccd1" }}>
            ...
            <div className="flex items-center gap-2 px-4 pb-2.5 pt-2 flex-wrap"
              style={{ borderTop:"1px solid rgba(0,0,0,0.1)", background:"#c0c2c8" }}>
