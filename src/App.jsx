...
import { AuthModal } from "./components/Shared.jsx";
...
// ── About Tab (self-contained — no Shared.jsx dependency) ───────────────────
function AboutTab() {
  ...
  async function submit() {
    ...
    try {
      const r = await fetch("https://formspree.io/f/xlgoezry", {
        method:"POST",
        headers:{ "Content-Type":"application/json", Accept:"application/json" },
        body: JSON.stringify({ name:feedback.name||"Anonymous", type:feedback.type, message:feedback.message }),
      });
      if (r.ok) setSubmitted(true);
      else setSendError("Something went wrong.");
    } catch { setSendError("No connection."); }
    finally { setSending(false); }
  }

  const card = { borderRadius:16, padding:20, background:"#0f1b2d", border:"1px solid rgba(255,255,255,0.1)", marginBottom:20 };
  const inp  = { width:"100%", borderRadius:10, padding:"10px 12px", fontSize:13, background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", color:"#fff", fontFamily:BRAND.fonts.body, boxSizing:"border-box" };

  return (
    <div style={{ overflowY:"auto", flex:1, fontFamily:BRAND.fonts.body }}>
      <div style={{ maxWidth:640, margin:"0 auto", padding:"24px 16px 48px" }}>

        {/* Hero */}
        <div style={{ ...card, textAlign:"center", padding:"36px 24px",
          background:"linear-gradient(135deg,#0f1b2d,#1a2d1a)" }}>
          <div style={{ fontSize:56, marginBottom:8 }}>⚽</div>
          <div style={{ fontFamily:BRAND.fonts.display, fontSize:28, fontWeight:900, color:"#fff", letterSpacing:2, marginBottom:6 }}>
            FCROSTER.COM
          </div>
          <div style={{ fontSize:11, color:"#94a3b8", letterSpacing:3, fontWeight:700, marginBottom:14 }}>
            CONNECT · ORGANIZE · COMPETE
          </div>
          <p style={{ fontSize:14, color:"rgba(255,255,255,0.65)", lineHeight:1.7, maxWidth:440, margin:"0 auto" }}>
            Built by footballers, for footballers. One platform for lineups, rosters, tactics, and community.
          </p>
        </div>

        {/* Who it's for */}
        <div style={{ marginBottom:20 }}>
          <div style={{ fontSize:10, fontWeight:900, letterSpacing:3, textTransform:"uppercase", color:BRAND.colors.yellow, marginBottom:12, display:"flex", alignItems:"center", gap:10 }}>
            Who It&apos;s For <div style={{ flex:1, height:1, background:"rgba(255,255,255,0.1)" }}/>
          </div>
          <div style={{ display:"flex", gap:10 }}>
            {[["⚽","Players"],["🎽","Coaches"],["🏆","Leagues"]].map(([icon,title]) => (
              <div key={title} style={{ flex:1, borderRadius:12, padding:"14px 8px", textAlign:"center",
                background:"#0f1b2d", border:"1px solid rgba(255,255,255,0.1)" }}>
        ...
        <div style={{ marginBottom:20 }}>
          <div style={{ fontSize:10, fontWeight:900, letterSpacing:3, textTransform:"uppercase", color:BRAND.colors.yellow, marginBottom:12, display:"flex", alignItems:"center", gap:10 }}>
            What You Get <div style={{ flex:1, height:1, background:"rgba(255,255,255,0.1)" }}/>
          </div>
          {[
            ["🗂️","Formation Builder","5v5, 7v7, 9v9 and 11v11 with all major formations"],
            ["🧠","Playmaker Tools","Draw runs, passes, and arrows on the pitch"],
            ["📋","Roster Manager","Name starters and subs, copy roster to clipboard"],
            ["🌍","Community Forum","Tactics, coaching tips, and football debate"],
            ["👤","Player Profiles","Stats, position history, and privacy controls"],
            ["📥","Download Lineups","Export your formation as a shareable image"],
          ].map(([icon,title,desc]) => (
            <div key={title} style={{ display:"flex", alignItems:"flex-start", gap:14, padding:"12px 14px",
              borderRadius:12, background:"#0f1b2d", border:"1px solid rgba(255,255,255,0.08)", marginBottom:8 }}>
        ...
        <div style={{ marginBottom:20 }}>
          <div style={{ fontSize:10, fontWeight:900, letterSpacing:3, textTransform:"uppercase", color:BRAND.colors.yellow, marginBottom:12, display:"flex", alignItems:"center", gap:10 }}>
            Share Your Thoughts <div style={{ flex:1, height:1, background:"rgba(255,255,255,0.1)" }}/>
          </div>
          <div style={card}>
            {submitted ? (
              <div style={{ textAlign:"center", padding:"12px 0" }}>
                <div style={{ fontSize:40, marginBottom:10 }}>🙏</div>
                <div style={{ fontWeight:700, color:"#fff", marginBottom:8 }}>Thanks for your feedback!</div>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                <p style={{ fontSize:12, color:"rgba(255,255,255,0.55)", lineHeight:1.6, margin:0 }}>
                  Got an idea or found something to improve? Drop us a note.
                </p>
                <div style={{ display:"flex", gap:8 }}>
                  {[["suggestion","💡 Suggestion"],["bug","🐛 Bug"],["other","💬 Other"]].map(([v,l]) => (
                    <button key={v} onClick={() => setFeedback(f => ({ ...f, type:v }))}
                      style={{ flex:1, padding:"8px 4px", borderRadius:10, fontSize:11, fontWeight:700, cursor:"pointer",
                        border:`1px solid ${feedback.type===v ? BRAND.colors.yellow : "rgba(255,255,255,0.1)"}`,
                        background: feedback.type===v ? `${BRAND.colors.yellow}18` : "transparent",
                        color: feedback.type===v ? BRAND.colors.yellow : "#6b7280" }}>
                      {l}
                    </button>
                  ))}
                </div>
                <input maxLength={80} placeholder="Your name (optional)" value={feedback.name}
                  onChange={e => setFeedback(f => ({ ...f, name:e.target.value }))} style={inp}/>
                ...
                <button onClick={submit} disabled={!feedback.message.trim() || sending}
                  style={{ padding:"12px", borderRadius:10, fontSize:14, fontWeight:700, border:"none",
                    cursor: feedback.message.trim() && !sending ? "pointer" : "not-allowed",
                    background: feedback.message.trim() && !sending ? BRAND.colors.yellow : "rgba(255,255,255,0.05)",
                    color: feedback.message.trim() && !sending ? "#111" : "#4b5563" }}>
                  {sending ? "Sending…" : "Send Feedback ✦"}
                </button>
                {sendError && <p style={{ fontSize:12, color:"#f87171", textAlign:"center", margin:0 }}>{sendError}</p>}
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ textAlign:"center", paddingBottom:8 }}>
          <button onClick={() => setShowLegal(true)}
            style={{ fontSize:11, fontWeight:700, letterSpacing:2, textTransform:"uppercase",
              background:"none", border:"none", cursor:"pointer", color:"#475569", marginBottom:8 }}>
            Legal Disclaimer
          </button>
          <div style={{ fontSize:11, color:"#374151" }}>© 2025 FCRoster.com · Connect. Organize. Compete.</div>
        </div>
      </div>

      {/* Legal modal */}
      {showLegal && (
        <div onClick={() => setShowLegal(false)}
          ...
          <div onClick={e => e.stopPropagation()}
            style={{ width:"100%", maxWidth:540, borderRadius:16, background:"#0f172a",
              border:"1px solid rgba(255,255,255,0.1)", maxHeight:"80vh", display:"flex", flexDirection:"column" }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between",
              padding:"16px 20px", borderBottom:"1px solid rgba(255,255,255,0.08)", flexShrink:0 }}>
              <div style={{ fontFamily:BRAND.fonts.display, fontSize:15, fontWeight:900, color:"#fff", letterSpacing:2 }}>
                LEGAL DISCLAIMER
              </div>
              <button onClick={() => setShowLegal(false)}
                style={{ background:"none", border:"none", cursor:"pointer", color:"rgba(255,255,255,0.5)", fontSize:20 }}>✕</button>
            </div>
            <div style={{ overflowY:"auto", padding:20, fontSize:12, color:"rgba(255,255,255,0.65)", lineHeight:1.7 }}>
              {[
                ["No Warranty","FCRoster.com is provided 'as is' without warranties of any kind, including merchantability or fitness for a particular purpose."],
                ["Limitation of Liability","We are not liable for any damages arising from your use of the service."],
                ["User Responsibility","You are responsible for all content you submit and must comply with applicable laws."],
                ["Intellectual Property","All branding and content belong to FCRoster.com. 'Connect. Organize. Compete.' is a proprietary mark."],
                ["Changes","We may update these terms at any time. Continued use constitutes acceptance."],
              ].map(([t,b]) => (
                <div key={t} style={{ marginBottom:14 }}>
                  <div style={{ fontWeight:700, color:"rgba(255,255,255,0.9)", marginBottom:3 }}>{t}</div>
                  <p style={{ margin:0 }}>{b}</p>
                </div>
              ))}
              <p style={{ fontSize:10, color:"rgba(255,255,255,0.3)", marginTop:12 }}>© 2025 FCRoster.com. All rights reserved.</p>
            </div>
          </div>
        </div>
      )}
