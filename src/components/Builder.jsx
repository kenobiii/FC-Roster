...
function FormationDiagram({ formation, format = 11 }) {
  ...
  return (
    <div style={{ position:"relative", borderRadius:"0.75rem", overflow:"hidden", background:"radial-gradient(ellipse at 50% 40%, #2d7a3a, #1a5c2e 70%, #0d3320)", aspectRatio:"2/3", width:"100%" }}>
    ...
    <>
      <div ref={exportRef} className="flex flex-col items-center gap-3 px-6 pt-4 pb-6 rounded-2xl" style={{ background:"#030712" }}>
        ...
        <div style={{ padding:"30px 20px 60px 20px", width:"100%", display:"flex", justifyContent:"center" }}>
          <div ref={pitchRef} className="rounded-2xl shadow-2xl"
            style={{ position:"relative", width:pitchSize.w, aspectRatio:"2/3", background:pitchBg, boxShadow:`0 0 60px rgba(0,0,0,0.5), 0 20px 60px rgba(0,0,0,0.7)`, overflow:"visible", flexShrink:0, cursor: dragging ? "grabbing" : "default", touchAction: moveMode ? "none" : "pan-y" }}
            ...
            <div style={{ position:"absolute", inset:0, borderRadius:"1rem", overflow:"hidden" }}><PitchLines/></div>
            {players.map((p,i) => (
