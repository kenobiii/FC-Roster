...
function BuilderLayout({
  ...
  const playmakeContent = (
    <div style={{ padding:"0 16px 20px 16px", display:"flex", flexDirection:"column", gap:10,
      borderTop:"1px solid rgba(255,255,255,0.08)" }}>

      {/* ── Player Movement ── */}
      <div style={{ paddingTop:14, paddingBottom:6, fontSize:10, fontWeight:900,
        letterSpacing:2, textTransform:"uppercase", color:"#94a3b8" }}>Player Movement</div>

      <button onClick={() => setMoveMode(v => !v)}
        style={{ display:"flex", alignItems:"center", justifyContent:"space-between",
          padding:"14px 16px", borderRadius:14, border:`1px solid ${moveMode ? "#6366f1" : "rgba(255,255,255,0.1)"}`,
          background: moveMode ? "rgba(99,102,241,0.15)" : "rgba(255,255,255,0.04)",
          cursor:"pointer", fontFamily:BRAND.fonts.body }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          <span style={{ fontSize:20 }}>🕹️</span>
          <span style={{ fontSize:14, fontWeight:700, color: moveMode ? "#a5b4fc" : "#cbd5e1" }}>Move Positions</span>
        </div>
        ...
        return (
          <button key={m} onClick={() => setDrawMode(isOn ? null : m)}
            style={{ display:"flex", alignItems:"center", justifyContent:"space-between",
              ...
              background: isOn ? `${color}14` : "rgba(255,255,255,0.04)",
              cursor:"pointer", fontFamily:BRAND.fonts.body }}>
            <div style={{ display:"flex", alignItems:"center", gap:12 }}>
              <svg width="36" height="16" viewBox="0 0 32 14">
              ...
              <span style={{ fontSize:14, fontWeight:700, color: isOn ? color : "#cbd5e1" }}>{label}</span>
            </div>
      ...
      <button onClick={() => setDrawMode(drawMode==="ball" ? null : "ball")}
        style={{ display:"flex", alignItems:"center", gap:12, padding:"14px 16px", borderRadius:14,
          border:`2px solid ${drawMode==="ball" ? "rgba(255,255,255,0.6)" : "rgba(255,255,255,0.1)"}`,
          background: drawMode==="ball" ? "rgba(255,255,255,0.1)" : "rgba(255,255,255,0.04)",
          cursor:"pointer", fontFamily:BRAND.fonts.body }}>
        <span style={{ fontSize:24, lineHeight:1,
          filter: drawMode==="ball" ? "drop-shadow(0 0 4px rgba(255,255,255,0.6))" : "none" }}>⚽</span>
        <span style={{ fontSize:14, fontWeight:700, color: drawMode==="ball" ? "#fff" : "#cbd5e1" }}>Drop Ball</span>
      </button>
      ...
      <button onClick={() => setShowOpposition(v => !v)}
        style={{ display:"flex", alignItems:"center", justifyContent:"space-between",
          padding:"14px 16px", borderRadius:14,
          border:`1px solid ${showOpposition ? "#ef4444" : "rgba(255,255,255,0.1)"}`,
          background: showOpposition ? "rgba(239,68,68,0.1)" : "rgba(255,255,255,0.04)",
          cursor:"pointer", fontFamily:BRAND.fonts.body }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <span style={{ fontSize:14, fontWeight:700, color: showOpposition ? "#ef4444" : "#cbd5e1" }}>Show Opposition</span>
        </div>
  ...
  const PlaymakerPanel = (
    <div style={{ borderRadius:16, overflow:"hidden", background:"#0f1b2d", border:`2px solid ${BRAND.colors.yellow}`, boxShadow:`0 0 18px ${BRAND.colors.yellow}30` }}>
      <div style={{ display:"flex", alignItems:"center", gap:8, padding:"12px 16px", background:"rgba(245,197,24,0.10)" }}>
        <span style={{ fontSize:16 }}>🧠</span>
        <span style={{ color:BRAND.colors.yellow, fontFamily:BRAND.fonts.display, letterSpacing:2, fontSize:18, fontWeight:900 }}>PLAYMAKER MENU 🔥</span>
      </div>
      {playmakeContent}
    </div>
  );
    ...
    <div style={{ borderRadius:20, overflow:"hidden", width:"100%", background:"#0f1b2d",
      ...
      <button onClick={handleExport} disabled={exporting}
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          ...
          <span style={{ color:"#fff", fontFamily:BRAND.fonts.display, letterSpacing:2, fontSize:18, fontWeight:900 }}>
    ...
    <div ref={containerRef} style={{ flex:1, display:"flex", minHeight:0, height:"100%" }}>
      {pitchSize.isDesktop ? (
        ...
        <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:12,
          padding:"12px 12px 20px 12px", overflowY:"auto", WebkitOverflowScrolling:"touch" }}>
          {Pitch}
          <div style={{ display:"flex", flexDirection:"column", gap:12, width:"100%" }}>

            {/* Playmaker always open, rounded to match Download/Roster */}
            <div style={{ borderRadius:20, overflow:"hidden", background:"#0f1b2d",
              border:"2px solid " + BRAND.colors.yellow,
              boxShadow:"0 0 24px " + BRAND.colors.yellow + "35" }}>
              <div style={{ display:"flex", alignItems:"center", gap:10,
                padding:"16px 20px", background:"rgba(245,197,24,0.10)" }}>
                <span style={{ fontSize:20 }}>🧠</span>
                <span style={{ color:BRAND.colors.yellow, fontFamily:BRAND.fonts.display,
                  letterSpacing:2, fontSize:18, fontWeight:900 }}>PLAYMAKER MENU 🔥</span>
              </div>
              {playmakeContent}
            </div>
